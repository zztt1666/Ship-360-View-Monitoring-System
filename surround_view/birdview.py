...
class BirdView(BaseThread):
    ...
    def load_weights_and_masks(self, weights_image, masks_image):
        # weights.png / masks.png 来自 run_get_weight_matrices.py
        # 其中 RGBA 四个通道分别存四个重叠区域的权重 / mask
    ...
    def stitch_all_parts(self):
        # front, back, left, right 四幅投影图按照固定区域拷贝到最终鸟瞰图中
        # 中间大块直接拷贝，四个角上的重叠区使用权重融合
    ...
    def make_luminance_balance(self):
        # 根据四个重叠区域的亮度统计，分别调整四路图像各通道亮度
        # 目的不是严格校准颜色，而是减轻拼接接缝处的明暗跳变
